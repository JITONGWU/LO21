...
QString Couple::AfficherCouple(){
    QString id1 = getX()->getId();
    QString id2 = getY()->getId();
    QString s="note:"+id1+"-> note:"+id2;
    return s;
}
bool Reference::supprimeroupas(QString id){
    for(int i=0;i<nbCouples;i++){
        if(couples[i]->getX()->getId()==id||couples[i]->getY()->getId()==id)
            return false;
    }
