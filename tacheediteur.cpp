...
if(restaurer<0) { rest->setVisible(false);}
    ...
    QObject::connect(echeance,SIGNAL(dateTimeChanged(QDateTime)),this,SLOT(activerSave()));
    ...
    for(NotesManager::Iterator it=NotesManager::getManager().getIterator();!it.isDone();it.next()){
        if(it.current().getEmp()=="A"){
            ...
            if(Reference::getRef()->supprimeroupas(it.current().getId())){
                 ...
                 {
                    SupprimertousTache2(it.current().getId());
                 }
                 else{message.close();}
            }
         }
...
void TacheEditeur::desarchiver(){
    NotesManager::getManager().desarchiver(tache->getId());
}

void TacheEditeur::SupprimertousTache2(QString id){
