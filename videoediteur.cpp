    ...
    //initialisation de image avec le parametre im
{
   ...
   desarchieve->setEnabled(true);
    ...
    QObject::connect(vfile,SIGNAL(textEdited(QString)),this,SLOT(getFile()));
...
void VideoEditeur::saveVideo(){
    ...
    if(restaurer >=0) {
        ...
        if(newVid==false) {
            ...
            static_cast<Video*>(vidTemp)->setDesc(desc->text());
...
void VideoEditeur::getFile(){/////////////////
    QString filename = QFileDialog::getOpenFileName(this,tr("Open video"), "/home/new video", tr("Image Files (*.mp4 *.wmv)"));
